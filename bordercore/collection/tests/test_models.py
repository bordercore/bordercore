...
def test_sort_collection(collection, blob_image_factory, blob_pdf_factory):
    ...
    so = SortOrderCollectionBlob.objects.get(collection=collection[0], blob=blob_pdf_factory[0])
    ...
    so = SortOrderCollectionBlob.objects.get(collection=collection[0], blob=blob_image_factory[0])
    SortOrderCollectionBlob.reorder(so, 1)
    so = SortOrderCollectionBlob.objects.filter(collection=collection[0])
    assert so[0].blob.id == blob_image_factory[0].id
    assert so[1].blob.id == blob_pdf_factory[0].id
